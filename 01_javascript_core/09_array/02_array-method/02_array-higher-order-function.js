...
// 다른 정렬 기준을 사용하려면 compare 함수를 인수로 전달해야한다.
console.log(`정렬 후 numbers : ${numbers}`);

// 숫자 오름차순 정렬에 사용 될 function
function compare(a,b){
    // if(a>b) return 1; //양수 반환 -> 순서변경
...
//각 요소별로 *10한 값을 콘솔에 출력하고 싶다면..

numbers.forEach(item => console.log(item*10));

//Array.prototype.map : 배열요소 전체를 대상으로 콜백 함수 호출 후 반환 값들로 구성 된 새로운 배열 반환
...
//Array.prototype.reduce : 배열을 순회하며 각 요소에 대항 이전의 콜백함수 실행 반환 값을
...
let result = [1,11,3,2,4].some(item => item>10);
...
result = [1,11,3,2,4].every(item => item>0);
console.log(`result : ${result}`);
